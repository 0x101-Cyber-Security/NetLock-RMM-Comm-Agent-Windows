                ...
                foreach (var job in Directory.GetFiles(Application_Paths.program_data_microsoft_defender_antivirus_scan_jobs))
                {
                    ...
                    if (execute)
                    {
                        ...
                        if (job_item.scan_type == 0) // quick scan
                        {
                            ...
                            result = PowerShell.Execute_Command("Microsoft_Defender_AntiVirus.Scan_Job_Scheduler.Check_Execution.Execute_Quickscan", "Start-MpScan -ScanType QuickScan", 30);
                        }
                        else if (job_item.scan_type == 1) // full scan
                        {
                            ...
                            result = PowerShell.Execute_Command("Microsoft_Defender_AntiVirus.Scan_Job_Scheduler.Check_Execution.Execute_Fullscan", "Start-MpScan -ScanType FullScan", 30);
                        }
                        ...
                        if (Service.language == "en-US")
                            Events.Logger.Insert_Event("0", "Microsoft Defender Antivirus", "Scan job completed. NetLock report.", "Scan job: " + job_item.name + Environment.NewLine + Environment.NewLine + "Result: " + result, 2, 0);
                        else if (Service.language == "de-DE")
                            Events.Logger.Insert_Event("0", "Microsoft Defender Antivirus", "Scanauftrag fertiggestellt. NetLock Bericht", "Scan job: " + job_item.name + Environment.NewLine + Environment.NewLine + "Ergebnis: " + result, 2, 1);
